...
def format_issue_body(issue, attach_names, options):
    ...
    skip_user = reporter and (reporter.get('username') or reporter['nickname']) == options.bb_skip
...
def format_comment_body(comment, options):
    ...
    skip_user = author and (author.get('username') or author['nickname']) == options.bb_skip
...
def format_user(user, options):
    ...
    if user is None:
        return "Anonymous"
    bb_user = "Bitbucket: [{0}](https://bitbucket.org/{0})".format(user.get('username') or user['nickname'])
    gh_username = _gh_username(user.get('username') or user['nickname'], options.users, options.gh_auth)
